...
5. **Scoring** -- Every contract with volume >= 10 is scored:
   - Load the contract's prior 20 trading days from the database
   - Fetch upcoming earnings date for the underlying (Polygon financials endpoint)
   - Compute z-scores for 11 factors (see below)
   - Apply weighted sum, normalize to 0-10 scale
   - Check the "already priced in" gate
6. **Alerting** -- If composite score >= 7.5 and not priced in: dedup check, fetch relevant news, format HTML email, send via Gmail SMTP.
7. **Sleep** -- Wait 15 minutes (matching Polygon's data delay), then repeat.
...
| IV Spike | `iv_z` | 0.13 | Implied volatility vs its own 20-day baseline; captures the market pricing in an event independent of volume |
...
| Sweep Detection | `sweep_z` | 0.10 | Volume extremity (proxy for sweep-like behaviour; contributes only when volume > 2 sigma) |
...
| OI Change | `oi_z` | 0.07 | Day-over-day open interest delta vs baseline. Note: OI is always the prior-day settlement figure (OCC calculates after close), so this is a lagging indicator |
...
A contract needs at least 5 days of historical data to compute a baseline. With fewer, the z-score defaults to 0 for that factor (contributes nothing). The backfilled data (~60 trading days from Polygon flat files) means most active contracts have robust baselines from day one.

## Alert Triggers

An alert fires when ALL of these are true:
- Composite score >= `ANOMALY_ALERT_MIN_SCORE` (default 7.5, configurable in `.env`)
- The contract is NOT already priced in
...
| `ANOMALY_ALERT_MIN_SCORE` | Composite score threshold for alerts | 7.5 |
...
|-------|---------|----------------|
| `options_snapshots` | Per-contract daily snapshot (OHLCV, greeks, OI, bid/ask) | ~7M |
| `stock_snapshots` | Per-ticker daily stock data | ~261K |
| `scoring_results` | Composite score + factor JSONB for scored contracts | 0 (populates on first live scan) |
| `alerts_sent` | Log of every alert sent/failed/suppressed | 0 |
| `backtest_runs` | Backtest metadata and results | 0 |
...
- [ ] **Run the scanner during a live market session** (next trading day: Monday April 6). Watch the logs, verify scoring results populate in the dashboard, confirm alert emails arrive if anything scores >= 7.5

### Short-term (1-2 weeks of live data)

- [ ] **Lower the alert threshold temporarily** to `5.0` in `.env` to see what the scorer is flagging. Review the factor breakdowns to check whether the signals make intuitive sense. Raise it back once you're satisfied.
- [ ] **Calibrate the composite score scaling factor** (currently `* 2.0` in `composite.py` line 87). After a week of data, look at the distribution of scores in the dashboard. If most scores cluster near 0-2 and nothing reaches 7.5, the scaling factor needs to increase. If too many scores hit 7.5+, decrease it or raise the threshold.
...
- [ ] **Implement real sweep detection** -- The current `sweep_z` factor is a volume proxy (only contributes when volume > 2 sigma). True sweep detection requires trade-level condition codes and multi-exchange fill analysis, which Polygon provides via websockets or trade endpoints. This is the highest-value improvement to signal quality.
